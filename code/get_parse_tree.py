...
if __name__ == '__main__':
    option=1 #0=craete parse tree, input to that is utterance parsed transcripts
    if option==0:
        #need to run the corenlp parsser in machine
        _parse_corpus(path+'participant_utterance_adrc.csv','utt','file','adrc')  # params: felename, utterace column, file column, database name
    else:
        df=pd.read_csv(path+"adrc_tags_4.csv")
        print(df['pos_freq'][3])
        # print(remove_non_ascii( 'Thats all your pay was a free meal.'))
