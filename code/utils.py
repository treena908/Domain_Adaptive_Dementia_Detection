...
from tensorflow.keras.layers import Layer, Dense, Flatten, Input,Activation,BatchNormalization,Dropout
...
def get_default_encoder(name="encoder", state=None):
    """
    ...
    if state is not None:
        ...
        model.add(Dense(58, activation="relu"),name=name)
    return model


def get_default_task(activation=None, name="task", state=None):
    """
    ...
    model = Sequential([
        Flatten(),
        Dense(58,kernel_regularizer=l2(0.001)),
        Activation('relu'),
        Dropout(0.5),
        ...
        Activation('relu'),
        Dropout(0.5),
        Dense(2, activation='softmax')],name=name)
...
def get_default_discriminator(name="domain", state=None):
    """
    Return a tensorflow Model of two hidden layers
    with 10 neurons each and relu activations. The
