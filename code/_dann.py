...
from base import BaseAdaptDeep, make_insert_doc
...
and V. Lempitsky. "Domain-adversarial training of neural networks". In JMLR, 2016.
    ...
    def __init__(self,
        ...
        self.task_optimizer = tf.keras.optimizers.legacy.Adam(learning_rate=0.01)
        self.domain_optimizer = tf.keras.optimizers.legacy.Adam(learning_rate=0.01)
        self.encoder_optimizer = tf.keras.optimizers.legacy.Adam(learning_rate=0.1)  # fe_lr=0.0005
    ...
    def train_step(self, data):
        ...
        with tf.GradientTape() as task_tape, tf.GradientTape() as enc_tape, tf.GradientTape() as disc_tape:
            ...
            # ys_pred = tf.reshape(ys_pred, [32,2])
        ...
        # Update weights
        self.task_optimizer.apply_gradients(zip(gradients_task, trainable_vars_task))
        self.encoder_optimizer.apply_gradients(zip(gradients_enc, trainable_vars_enc))
        self.domain_optimizer.apply_gradients(zip(gradients_disc, trainable_vars_disc))
