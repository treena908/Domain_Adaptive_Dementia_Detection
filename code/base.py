...
from metrics import normalized_linear_discrepancy
    ...
    def fit(self, X, y=None, Xt=None, yt=None, domains=None, **fit_params):
        ...
        if not isinstance(X, tf.data.Dataset):
            ...
            # Multisource
                ...
                # temp_x=tuple(tf.data.Dataset.from_tensor_slices(X[0]).repeat(repeats[0]))+tuple(tf.data.Dataset.from_tensor_slices(X[1]).repeat(repeats[1]))
                # dataset_Xs = tf.data.Dataset.zip(temp_x)
                # temp_y=tuple(tf.data.Dataset.from_tensor_slices(y[0]).repeat(repeats[0]))+tuple(tf.data.Dataset.from_tensor_slices(y[1]).repeat(repeats[1]))
                # dataset_ys=tf.data.Dataset.zip(temp_y)
